...
import { DOCS_PAGE_SIZE, DOC_STATUS_CFG } from "../../analysis/displayConstants";
import { ChecklistRow, FindingCard } from "./shared.jsx";

export function AmendmentChainTree({ docId, docNameMap, amendmentLink, parentDoc }) {
  return (
    <div className="bg-muted/30 rounded-lg p-3">
      <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2">Amendment Chain</h4>
      <div className="text-xs space-y-1">
        ...
        <div className="flex items-center gap-1.5 pl-5">
          ...
          <span className="text-muted-foreground">({amendmentLink?.amendment_type || "modifies"}, {Math.round((amendmentLink?.confidence || 0) * 100)}%)</span>
        </div>
      </div>
    </div>
  );
...
export function DocumentAnalysisCard({ doc, checklist, findings, docNameMap, onCitationClick, roomId, onFindingUpdated }) {
  ...
  const docStatus = highFindings > 0 ? "conflict" : openFindings > 0 ? "warning" : docFindings.length > 0 ? "ok" : "neutral";
  const statusCfg = DOC_STATUS_CFG[docStatus];
  const StatusIcon = statusCfg.icon;

  const topFindings = useMemo(() => docFindings
    .filter((finding) => finding.status === "open")
    .sort((a, b) => ({ high: 0, medium: 1, low: 2 }[a.severity] ?? 2) - ({ high: 0, medium: 1, low: 2 }[b.severity] ?? 2))
    .slice(0, 2), [docFindings]);

  return (
    <div className="bg-card border rounded-xl overflow-hidden shadow-sm">
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/20 transition-colors" onClick={() => setExpanded((value) => !value)}>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {hasAnalysisData && (
            <>
              {checklistTotal > 0 && <span className="text-xs text-muted-foreground">{checklistCovered}/{checklistTotal} checklist</span>}
              {openFindings > 0 && (
          ...
          {!hasAnalysisData && <span className="text-xs text-muted-foreground">No analysis data</span>}
      ...
      {expanded && (
        <div className="border-t px-4 py-3 space-y-4">
          {amendmentParent && (
            <AmendmentChainTree docId={docId} docNameMap={docNameMap} amendmentLink={amendmentLink} parentDoc={parentDoc} />
          )}

          {topFindings.length > 0 && (
            <div className="bg-muted/30 rounded-lg p-3 space-y-2">
              <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Key Issues</h4>
          ...
          {docChecklist.length > 0 && (
            <div>
              <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2">Checklist Items ({docChecklist.length})</h4>
          ...
          {docFindings.length > 0 && (
            <div>
              <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2">Findings ({docFindings.length})</h4>
          ...
          {!hasAnalysisData && <p className="text-sm text-muted-foreground text-center py-4">No analysis data extracted from this document yet.</p>}
        </div>
      )}
    </div>
  );
...
export function ByDocumentTab({ docs, checklist, findings, docNameMap, onCitationClick, roomId, setFindings }) {
  const [docsPage, setDocsPage] = useState(1);

  const sortedDocs = useMemo(() => [...docs].sort((a, b) => {
    const aFindings = findings.filter((finding) => finding.source_document_id === a.document_id || finding.evidence_spans?.some((span) => span.source_document_id === a.document_id)).length;
    const bFindings = findings.filter((finding) => finding.source_document_id === b.document_id || finding.evidence_spans?.some((span) => span.source_document_id === b.document_id)).length;
        ...
        <>
          {visibleDocs.map((doc) => (
            <DocumentAnalysisCard
              ...
              onCitationClick={onCitationClick}
              roomId={roomId}
              onFindingUpdated={(updated) => setFindings((prev) => prev.map((item) => (item.id === updated.id ? updated : item)))}
            />
          ))}
          {hasMore && (
            <button
              onClick={() => setDocsPage((page) => page + 1)}
