...
class V1RagPromptSet(RagPromptSet):
    ...
    def system_instructions_with_chunks(self) -> str:
        return (
            ...
            "CITATION RULES:\n"
            "- Cite every factual claim using [Sn:pN] — n is the sequential source number shown in the prompt, "
            "N is the page number.\n"
            "  Example: \"The Year-One Cap Rate is 8.11% [S1:p6].\"\n"
            ...
            "- Report ALL financial figures exactly as they appear in the source — including values "
            "labeled as 'Proposed', 'Pro Forma', 'Estimated', or 'Projected'. These are real data "
            "points in the document; do NOT say a value is absent because it is labeled as proposed "
            "or hypothetical. State the value and note the label (e.g., 'Proposed loan amount: $1,625,000').\n"
            ...
            "  Do NOT use [Sn:pN] citations for general context.\n"
            ...
            "- Do not correct, revisit, or reference earlier answers unless the user explicitly asks for a correction,"
    ...
    def comparison_system_instructions(self) -> str:
        return (
            ...
            "|--------|------------|------------|------------|\n"
            "| Cap Rate | 8.11% [S3:p6] | 6.50% [S11:p4] | +1.61% |\n\n"
            "TABLE FORMAT (3+ documents):\n"
            "| Metric | Document A | Document B | Document C |\n"
            "|--------|------------|------------|------------|\n"
            "| Cap Rate | 8.11% [S3:p6] | 6.50% [S11:p4] | 7.20% [S19:p9] |\n\n"
            "CITATION RULES:\n"
            "- Every number in the table must have a citation [Sn:pN] using the EXACT source numbers "
            "from the extracted facts above — do NOT renumber them sequentially (1, 2, 3...). "
            "Different documents will have different, non-consecutive Sn values.\n"
            "- If a metric is not found in a document, write **Not disclosed** in that cell — "
            "never leave it blank and never invent a value.\n"
            "- Do not use [Sn:pN] citations for general financial context.\n\n"
            "CONTENT RULES:\n"
            "- Only use property-specific data (prices, rents, NOI, cap rates, occupancy) from the provided content.\n"
            "- Treat each metric label literally. Do not substitute one metric for another "
            "(for example: never use NOI as Gross Potential Rent, never use Year One values as Current values, "
            "and never use Pro Forma values as Current values unless explicitly labeled that way).\n"
            "- Only compare or rank a metric when the cited value is explicitly present for that document. "
            "If a metric is missing for one document, write **Not disclosed** in the table and say the direct comparison "
            "is limited or unavailable.\n"
            "- For period-specific rows such as Current, Year One, or Pro Forma, include that row only when that same "
            "metric-period combination is explicitly available for each document being compared. Otherwise omit the row or "
            "mark the missing cell as **Not disclosed** — never backfill it from a different metric.\n"
            "- Do not infer undisclosed metrics or qualitative claims such as execution risk, superior management, "
            "or better returns unless the prompt provides direct supporting facts for that conclusion.\n"
            "- Before writing any comparative claim like higher, lower, better, worse, more upside, or less upside, "
            "verify that the cited numbers support that direction exactly.\n"
