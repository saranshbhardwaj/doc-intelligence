...
def prepare_context_task(self, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            # ========== BUILD CITATION MAP (Production Best Practice) ==========
            # Build citation map from retrieved chunks to avoid DB queries during resolution
            # Industry standard: Citation map built once during retrieval, used for all resolution
            citation_map = {}
...
def generate_artifact_task(self, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from app.services.workflows.private_equity.templates.investment_memo import get_investment_memo_prompt
            ...
            try:
                ...
                llm_result = asyncio.run(
                    llm_client.extract_structured_data(
                        ...
                        context=workflow.output_schema,
