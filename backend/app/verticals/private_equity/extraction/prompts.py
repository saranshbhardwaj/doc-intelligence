"""Extraction prompts - CIM extraction and batch summarization.

This module contains all prompts used in the extraction pipeline:
1. CIM_EXTRACTION_SYSTEM_PROMPT: System prompt for CIM extraction
2. SUMMARY_SYSTEM_PROMPT: System prompt for chunk summarization
3. BATCH_SUMMARY_PROMPT_TEMPLATE: Template for batch chunk summarization
4. Helper functions: create_extraction_prompt(), create_batch_summary_prompt()
"""

# ============================================================================
# CIM EXTRACTION PROMPTS
# ============================================================================
...
# ============================================================================
# SUMMARIZATION PROMPTS (for batch chunk summarization)
# ============================================================================

SUMMARY_SYSTEM_PROMPT = """Create ultra-concise summaries of document chunks, preserving numbers and citations.

**Task:**
Summarize each chunk in 1-2 sentences maximum. Keep only critical numbers and facts.
Remove all marketing fluff, generic statements, and redundant phrasing.

**Citation Format:**
Keep citations: "[D1:p2]", "[D3:p5]", etc.
Include inline: "Revenue: $111.9M [D1:p2]"

**Output:**
Terse summary with key numbers.
Format: "Key fact 1 [D1:p2]. Key fact 2 [D1:p3]. Key Numbers: $X, Y%"

**Do Not:**
Add analysis, explanations, or infer information.
"""
...
BATCH_SUMMARY_PROMPT_TEMPLATE = """Summarize each chunk below ultra-concisely. Keep numbers and citations.

{batch_text}

Format (exactly {chunk_count} summaries, 1-2 sentences each):

Chunk 1: [Terse 1-2 sentence summary with citations [D1:p2]]
Key Numbers: [numbers with citations]

Chunk 2: [Terse 1-2 sentence summary with citations [D1:p2]]
Key Numbers: [numbers with citations]
...
def create_batch_summary_prompt(chunks: list[dict]) -> str:
    ...
    Args:
        chunks: List of dicts with 'page' and 'text' keys
                (page is included for context but summaries are per-chunk)
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        text = chunk.get("text", "")
        # Include page number for context, but use chunk index for identification
        batch_sections.append(f"[Chunk {idx} - Page {page_num}]\n{text}")

    batch_text = "\n\n---CHUNK BREAK---\n\n".join(batch_sections)
    return BATCH_SUMMARY_PROMPT_TEMPLATE.format(batch_text=batch_text, chunk_count=len(chunks))
