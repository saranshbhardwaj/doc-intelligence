...
def detect_discrepancies(om_data: dict, rent_roll_data: dict, t12_data: dict) -> list[dict]:
    ...
    if om_gpr and rr_gpr:
        ...
        absolute_delta = abs(om_gpr - rr_gpr)
        if relative_delta >= 0.03 or absolute_delta >= 50_000:
            discrepancies.append({
                ...
                "severity": "warning",
                "note": f"GPR mismatch: OM ${om_gpr:,.0f} vs rent roll ${rr_gpr:,.0f} ({relative_delta:.1%} / ${absolute_delta:,.0f})",
            })
